...
| Distance filtering params on `POST /distance-matrix` | Supported on the distance-matrix endpoint | Not included | Same rationale as standalone distance. The filtering params exist on the `geocode --to` and `reverse --to` inline distance features where they make more sense for interactive use. |
