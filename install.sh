#!/bin/sh
# Geocodio CLI Install Script
# https://github.com/geocodio/geocodio-cli
#
# Usage:
...
# Configuration
GITHUB_REPO="geocodio/geocodio-cli"
BINARY_NAME="geocodio"
VERSION=""
...
Environment Variables:
    GEOCODIO_INSTALL_DIR     Override installation directory
EOF
                exit 0
                ;;
            *)
...
main() {
    ...
    # Version without 'v' prefix for archive name (GoReleaser convention)
    VERSION_NUM="${VERSION#v}"
    ARCHIVE_NAME="${BINARY_NAME}-cli_${VERSION_NUM}_${OS}_${ARCH}.tar.gz"
    ...
    # Find installation directory
    if [ -n "$GEOCODIO_INSTALL_DIR" ]; then
        INSTALL_DIR="$GEOCODIO_INSTALL_DIR"
    ...
    if [ -x "$INSTALL_DIR/$BINARY_NAME" ]; then
        success "Geocodio CLI installed successfully to $INSTALL_DIR/$BINARY_NAME"
