...
- **Concurrency explanation for queued lists.** `lists status` and `lists upload --watch` now explain, to stderr, why a list can stay in `ENQUEUED` longer than expected: Geocodio processes a limited number of spreadsheet jobs per account at once (1 on pay-as-you-go/Flex, 3 per dedicated instance on Unlimited), dashboard uploads count toward the same limit, and the job starts automatically once a slot frees up. There is no rejection or error to catch here -- `POST /lists` always succeeds -- so this is purely explanatory. The bundled agent skill (`skills/geocodio/SKILL.md`) documents the same behavior so an agent doesn't mistake a slow `ENQUEUED` for a stuck or failed job.
