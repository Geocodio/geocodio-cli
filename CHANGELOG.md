...
- **Concurrency explanation for queued lists.** `lists status` and `lists upload --watch` now explain, to stderr, why a list can stay in `ENQUEUED` longer than expected: Geocodio limits how many spreadsheet jobs one account can process at once, the limit depends on your plan, and dashboard uploads count toward it too -- the job starts automatically once a slot frees up. There is no rejection or error to catch here -- `POST /lists` always succeeds -- so this is purely explanatory, and it points to [the concurrency guide](https://www.geocod.io/guides/spreadsheet-concurrency) for your plan's actual limit rather than hardcoding numbers that would drift out of date. The bundled agent skill (`skills/geocodio/SKILL.md`) documents the same behavior so an agent doesn't mistake a slow `ENQUEUED` for a stuck or failed job.
